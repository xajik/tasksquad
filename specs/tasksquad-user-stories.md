...
| Symbol | Meaning |
|--------|---------|
| ✅ | Done — fully implemented (worker + portal) |
| ⚠️ | Partial — core logic exists but missing parts |
| ❌ | Not started |
...
| ID     | Status | Story | Acceptance Criteria |
|--------|:------:|-------|---------------------|
...
> **Notes:** Portal uses email/password only. Worker correctly upserts users by Firebase UID on first login. Social providers need to be enabled in Firebase console and wired up in the portal Auth page.
...
> **Notes:** TEAM-1 fully works (worker + portal gate). TEAM-2 through TEAM-9 have no worker routes or portal UI. Team context is currently stored in `localStorage` (single team only).
...
> **Notes:** AGENT-1/2/5 fully implemented. Token is scoped to team (not per-agent in current schema — `daemon_tokens` has `team_id`). AGENT-8 requires a Cloudflare Cron Trigger to sweep stale agents.

### 3.4 Task Messaging (Email-like)

| ID     | Status | Story | Acceptance Criteria |
|--------|:------:|-------|---------------------|
| TASK-1 | ⚠️ | As a team member, I want to compose a new task with To and CC fields (selecting agents and/or users) so I can send work to specific recipients. | Compose view allows selecting multiple recipients from a team roster (agents + users). To and CC are distinct. |
...
| TASK-7 | ⚠️ | As a team member, I want to see the agent's response (stdout/output from CLI execution) formatted in the thread so I can review results. | Agent response is rendered with code formatting / syntax highlighting where appropriate. |
...
> **Notes:** TASK-1 partial — single agent To field only, no CC, no user recipients. TASK-2 missing — compose form only has a `subject` field; no body textarea. TASK-7 partial — agent messages render in `monospace` but no syntax highlighting or markdown parsing.
...
| COMM-4 | ⚠️ | As the system, when an agent picks up a reply in an existing thread, I want to deliver only the new messages so the daemon has context to continue. | Agent receives incremental thread updates, not the full history on every ping. |

> **Notes:** COMM-1/2/3 fully implemented in worker (`daemon/heartbeat`, `daemon/session-open`, `daemon/session-close`). COMM-4 partial — heartbeat returns the pending task body but doesn't deliver incremental thread history for follow-up replies in a `waiting_input` task.

### 3.6 Dashboard & Navigation

| ID     | Status | Story | Acceptance Criteria |
|--------|:------:|-------|---------------------|
| DASH-1 | ⚠️ | As a user, I want a dashboard that shows my teams, recent tasks, and agent statuses so I have an at-a-glance overview. | Dashboard displays team cards, recent task activity, and agent health summary. |
...
> **Notes:** DASH-1 partial — there is a functional inbox + agents sidebar but no overview/summary page or team cards. DASH-2 blocked by TEAM-2 (no multi-team support). DASH-3 no `/profile` route.
...
> **Notes:** No `/pricing` route or `Pricing.tsx` component exists. No waitlist storage on the worker.
