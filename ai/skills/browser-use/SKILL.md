...
Always use "TaskSquad" profile: 
```bash
browser-use --profile "task-squad-browser" <command>
```

## Gmail Authentificatoin

You have access to Gmail account, use it for any authentificatoin in 3rd party services when required
...
browser-use --profile "TaskSquad" open <url>      # Real Chrome with Default profile (existing logins/cookies)
