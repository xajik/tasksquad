import { ArrowRight, User, Bot, Loader2, Server, MessageSquare, Save } from 'lucide-react'

export function AgentWorkflow() {
  const stages = [
    { icon: User, label: 'Create agent', color: 'bg-blue-500/10 text-blue-500' },
    { icon: Bot, label: 'Agent', color: 'bg-purple-500/10 text-purple-500' },
    { icon: Loader2, label: 'Heartbeat', color: 'bg-orange-500/10 text-orange-500', animate: 'animate-spin' },
    { icon: Server, label: 'Poll task', color: 'bg-green-500/10 text-green-500' },
    { icon: MessageSquare, label: 'Interact', color: 'bg-yellow-500/10 text-yellow-500' },
    { icon: Save, label: 'Save logs', color: 'bg-pink-500/10 text-pink-500' },
  ]
