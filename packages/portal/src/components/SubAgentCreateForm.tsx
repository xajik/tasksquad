...
export function SubAgentCreateForm({ onSubmit, onCancel, error }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="px-6 pb-4 flex flex-col gap-3 border-b">
      ...
      <Input placeholder="agent-name" value={name} onChange={e => setName(e.target.value)} className="font-mono" required />
      <Input placeholder="Description — when to invoke this agent" value={desc} onChange={e => setDesc(e.target.value)} />
      <Textarea
        placeholder="---&#10;name: my-agent&#10;description: When to invoke&#10;tools: Bash&#10;---&#10;# Instructions"
        value={content} onChange={e => setContent(e.target.value)}
        className="font-mono text-sm min-h-[150px]" required
      />
      <div className="text-[10px] text-muted-foreground bg-muted/30 p-2.5 rounded border border-dashed font-mono">
        ...
        {showExample && (
          <div className="space-y-0.5 opacity-80 mt-1.5">
            ---<br />name: my-agent<br />description: when to invoke<br />tools: Bash, Read, Edit<br />---<br /><br /># Instructions<br />1. First do X<br />2. Then do Y
          </div>
        )}
      </div>
